...
module.exports.controller = (app, io, socket_list) => {
  ...
  app.post("/api/app/login", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["email", "password", "dervice_token"], () => {
      ...
      db.query(
        ...
        [auth_token, reqObj.dervice_token, reqObj.email, reqObj.password, "1"],
        (err, result) => {
          if (err) {
          ...
          (err, result) => {
            ...
            if (result.length > 0) {
              ...
              db.query(
                ...
                [reqObj.username, reqObj.email, reqObj.password, auth_token, reqObj.dervice_token],
                (err, result) => {
                  if (err) {
                    helper.ThrowHtmlError(err, res);
                    return;
  ...
  app.post("/api/app/home", (req, res) => {
    ...
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          "SELECT `od`.`price` as `offer_price`, `od`.`start_date`, `od`.`end_date`, `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`brand_id`, `pd`.`type_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`,(CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" +
            image_base_url +
            "' ,'', `imd`.`image` ) ELSE '' END) AS `image`  , `cd`.`cat_name`,  `td`.`type_name`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav` FROM `offer_detail` AS `od` " +
            ...
            image_base_url +
            "' ,'', `imd`.`image` ) ELSE '' END) AS `image`, `cd`.`cat_name`,  `td`.`type_name`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav` FROM  `product_detail` AS `pd` " +
            "LEFT JOIN  `favorite_detail` AS `fd` ON  `pd`.`prod_id` = `fd`.`prod_id` AND `fd`.`user_id` = ? AND `fd`.`status`=  1 " +
            "INNER JOIN `image_detail` AS `imd` ON `pd`.`prod_id` = `imd`.`prod_id` AND `imd`.`status` = 1 " +
            "INNER JOIN `category_detail` AS `cd` ON `cd`.`cat_id` = `pd`.`cat_id` AND `cd`.`status` = 1 " +
            "INNER JOIN `type_detail` AS `td` ON `pd`.`type_id` = `td`.`type_id` AND `td`.`status` = 1 " +
            ...
            "INNER JOIN `category_detail` AS `cd` ON `cd`.`cat_id` = `pd`.`cat_id` AND `cd`.`status` = 1 " +
            "INNER JOIN `type_detail` AS `td` ON `pd`.`type_id` = `td`.`type_id` AND `td`.`status` = 1 " +
            "WHERE `pd`.`status` = ? GROUP BY `pd`.`prod_id` ORDER BY `pd`.`prod_id` ;",
          ["1", uObj.user_id, "1", uObj.user_id, "1", "1", "1", uObj.user_id, "1"],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
              return;
        ...
        helper.CheckParameterValid(res, reqObj, ["cat_id"], () => {
          db.query(
            "SELECT `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`brand_id`,  `pd`.`type_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, `pd`.`created_date`, `pd`.`modify_date`, `cd`.`cat_name`, IFNULL( `bd`.`brand_name`, '' ) AS `brand_name` , `td`.`type_name`, IFNULL(`od`.`price`, `pd`.`price` ) as `offer_price`, IFNULL(`od`.`start_date`,'') as `start_date`, IFNULL(`od`.`end_date`,'') as `end_date`, (CASE WHEN `od`.`offer_id` IS NOT NULL THEN 1 ELSE 0 END) AS `is_offer_active`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav`, (CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" +
              ...
              "LEFT JOIN `offer_detail` AS `od` ON `pd`.`prod_id` = `od`.`prod_id` AND `od`.`status` = 1 AND `od`.`start_date` <= NOW() AND `od`.`end_date` >= NOW() " +
              "INNER JOIN `type_detail` AS `td` ON `pd`.`type_id` = `td`.`type_id` " +
              " WHERE `cd`.`cat_id` = ? AND `cd`.`status` = '1' GROUP BY `pd`.`prod_id`  ",
            [reqObj.cat_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
    ...
    checkAccessToken(req.headers, res, (userObj) => {
      helper.CheckParameterValid(res, reqObj, ["cart_id", "prod_id", "new_qty"], () => {
        ...
        db.query(
          ...
          [reqObj.new_qty, status, reqObj.cart_id, reqObj.prod_id, userObj.user_id, "1"],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
              return;
              ...
              if (pResult.length > 0) {
                ...
                if (final_total > pResult[0].min_order_amount) {
                  ...
                  res.json({
                    ...
                    payload: result,
                    total: total,
                    deliver_price_amount: deliver_price_amount,
                    discount_amount: 0,
                    user_pay_price: final_total + deliver_price_amount,
  ...
  app.post("/api/app/add_delivery_address", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (userObj) => {
      helper.CheckParameterValid(
        res,
        reqObj,
        ["name", "type_name", "phone", "address", "city", "state", "postal_code"],
        () => {
          db.query(
  ...
  app.post("/api/app/update_delivery_address", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (userObj) => {
      helper.CheckParameterValid(
        res,
        reqObj,
        ["address_id", "name", "type_name", "phone", "address", "city", "state", "postal_code"],
        () => {
          db.query(
  ...
  app.post("/api/app/order_place", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (userObj) => {
      helper.CheckParameterValid(
        res,
        reqObj,
        ["address_id", "payment_type", "deliver_type", "pay_id", "promo_code_id"],
        () => {
          getUserCart(res, userObj.user_id, (result, total) => {
            if (result.length > 0) {
              db.query(
                ...
                [reqObj.pay_id, reqObj.promo_code_id, reqObj.address_id, userObj.user_id],
                (err, pResult) => {
                  if (err) {
                    helper.ThrowHtmlError(err, res);
                    return;
  ...
  app.post("/api/app/order_payment_transaction", (req, res) => {
    ...
    checkAccessToken(req.headers, res, (userObj) => {
      helper.CheckParameterValid(
        res,
        reqObj,
        ["order_id", "payment_transaction_id", "payment_status", "transaction_payload"],
        () => {
          db.query(
              ...
              if (result) {
                ...
                db.query(
                  ...
                  [
                    ...
                    "your order #" + reqObj.order_id + " payment " + message + ".",
                    "2",
                  ],
                  (err, iResult) => {
                    if (err) {
                ...
                db.query(
                  ...
                  [reqObj.payment_status == "1" ? "2" : "3", reqObj.order_id, userObj.user_id],
                  (err, uResult) => {
                    if (err) {
                      helper.ThrowHtmlError(err);
                      return;
    ...
    checkAccessToken(req.headers, res, (userObj) => {
      helper.CheckParameterValid(res, reqObj, ["order_id"], () => {
        db.query(
          ...
          [userObj.user_id, reqObj.order_id, userObj.user_id, reqObj.order_id, userObj.user_id],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
              return;
  ...
  app.post("/api/app/notification_list", (req, res) => {
    ...
    checkAccessToken(
      ...
      (userObj) => {
        db.query(
          "SELECT `notification_id`, `ref_id`, `title`, `message`, `notification_type`, `is_read`, `created_date` FROM `notification_detail` WHERE `user_id` = 2 AND `status` = 1 ORDER BY `notification_id` DESC",
          [userObj.user_id],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
    ...
    checkAccessToken(req.headers, res, (userObj) => {
      helper.CheckParameterValid(res, reqObj, ["username", "name", "mobile", "mobile_code"], () => {
        db.query(
          ...
          [reqObj.username, reqObj.name, reqObj.mobile, reqObj.mobile_code, userObj.user_id],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
              return;
            ...
            db.query(
              ...
              (err, uResult) => {
                ...
                if (uResult.affectedRows > 0) {
                  res.json({
                    status: "1",
                    payload: { user_id: result[0].user_id, reset_code: reset_code },
                    message: msg_success,
                  });
                } else {
                  res.json({
  ...
  function getUserCart(res, user_id, callback) {
    db.query(
      "SELECT `ucd`.`cart_id`, `ucd`.`user_id`, `ucd`.`prod_id`, `ucd`.`qty`, `ucd`.`size`, `pd`.`cat_id`, `pd`.`brand_id`, `pd`.`type_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, `pd`.`created_date`, `pd`.`modify_date`, `cd`.`cat_name`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav` , IFNULL( `bd`.`brand_name`, '' ) AS `brand_name` , `td`.`type_name`, IFNULL(`od`.`price`, `pd`.`price` ) as `offer_price`, IFNULL(`od`.`start_date`,'') as `start_date`, IFNULL(`od`.`end_date`,'') as `end_date`, (CASE WHEN `od`.`offer_id` IS NOT NULL THEN 1 ELSE 0 END) AS `is_offer_active`, (CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" +
        image_base_url +
