...
module.exports.controller = (app, io, socket_list) => {
  ...
  app.post("/api/admin/brand_add", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["brand_name"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "SELECT `brand_id`, `brand_name` FROM `brand_detail` WHERE `brand_name`  = ? AND `status` = ?",
            [reqObj.brand_name, "1"],
            (err, result) => {
              if (err) {
              ...
              if (result.length > 0) {
                ...
                db.query(
                  "INSERT INTO `brand_detail`( `brand_name`, `created_date`, `modify_date`) VALUES (?, NOW(), NOW())",
                  [reqObj.brand_name],
                  (err, result) => {
                    if (err) {
                      helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/brand_update", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["brand_id", "brand_name"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `brand_detail` SET `brand_name`= ?, `modify_date` = NOW() WHERE `brand_id`= ? AND `status` = ? ",
  ...
  app.post("/api/admin/brand_delete", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["brand_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `brand_detail` SET `status`= ?, `modify_date` = NOW() WHERE `brand_id`= ? ",
            ["2", reqObj.brand_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/brand_list", (req, res) => {
    ...
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          "SELECT `brand_id`, `brand_name` FROM `brand_detail` WHERE `status`= ? ",
          ["1"],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
      ...
      (uObj) => {
        db.query(
          "SELECT `cat_id`, `cat_name`,  (CASE WHEN `image` != '' THEN CONCAT('" +
            helper.ImagePath() +
            "','',`image`)  ELSE `image` END) AS `image` , `color` FROM `category_detail` WHERE `status`= ? ",
          ["1"],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
          ...
          helper.CheckParameterValid(res, files, ["image"], () => {
            ...
            fs.rename(files.image[0].path, newPath, (err) => {
              if (err) {
                ...
                db.query(
                  "INSERT INTO `type_detail`( `type_name`, `image`, `color`, `created_date`, `modify_date`) VALUES  (?,?,?, NOW(), NOW())",
    ...
    checkAccessToken(req.headers, res, (uObj) => {
      form.parse(req, (err, reqObj, files) => {
        ...
        helper.CheckParameterValid(res, reqObj, ["type_id", "type_name", "color"], () => {
          ...
          db.query(
            "UPDATE `type_detail` SET `type_name`=?," +
              condition +
              " `color`=?,`modify_date`=NOW() WHERE `type_id`= ? AND `status` = ? ",
  ...
  app.post("/api/admin/product_type_delete", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["type_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `type_detail` SET `status`= ?, `modify_date` = NOW() WHERE `type_id`= ? ",
            ["2", reqObj.type_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
      ...
      (uObj) => {
        db.query(
          "SELECT `type_id`, `type_name`,  (CASE WHEN `image` != '' THEN CONCAT('" +
            helper.ImagePath() +
            "','',`image`)  ELSE `image` END) AS `image` , `color` FROM `type_detail` WHERE `status`= ? ",
          ["1"],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
    ...
    helper.CheckParameterValid(
      ...
      () => {
        checkAccessToken(
          ...
          (uObj) => {
            db.query(
              "UPDATE `product_detail` SET `cat_id`=?,`brand_id`=?,`type_id`=?,`name`=?,`detail`=?,`unit_name`=?,`unit_value`=?,`nutrition_weight`=?,`price`=?, `modify_date`=NOW() WHERE  `prod_id`= ? AND `status` = ? ",
              [
                reqObj.cat_id,
                reqObj.brand_id,
                reqObj.type_id,
  ...
  app.post("/api/admin/product_delete", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["prod_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `product_detail` SET `status`=?,`modify_date`=NOW() WHERE  `prod_id`= ? AND `status` = ? ",
            ["2", reqObj.prod_id, "1"],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/product_nutrition_add", (req, res) => {
    ...
    helper.CheckParameterValid(
      res,
      reqObj,
      ["prod_id", "nutrition_name", "nutrition_value"],
      () => {
        checkAccessToken(
          req.headers,
          res,
          (uObj) => {
            db.query(
              "INSERT INTO `nutrition_detail`( `prod_id`, `nutrition_name`, `nutrition_value`, `created_date`, `modify_date`) VALUES (?,?,?, NOW(), NOW())",
  ...
  app.post("/api/admin/product_nutrition_update", (req, res) => {
    ...
    helper.CheckParameterValid(
      res,
      reqObj,
      ["prod_id", "nutrition_id", "nutrition_name", "nutrition_value"],
      () => {
        checkAccessToken(
          req.headers,
          res,
          (uObj) => {
            db.query(
              "UPDATE `nutrition_detail` SET `nutrition_name`= ?,`nutrition_value`= ?, `modify_date`= NOW() WHERE  `prod_id`= ? AND `nutrition_id` = ? AND `status` = ? ",
              [
                reqObj.nutrition_name,
                reqObj.nutrition_value,
                reqObj.prod_id,
                reqObj.nutrition_id,
                "1",
              ],
  ...
  app.post("/api/admin/product_nutrition_delete", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["prod_id", "nutrition_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `nutrition_detail` SET `status`= ?, `modify_date`= NOW() WHERE  `prod_id`= ? AND `nutrition_id` = ? AND `status` = ? ",
          ...
          helper.CheckParameterValid(res, files, ["image"], () => {
            ...
            fs.rename(files.image[0].path, newPath, (err) => {
              if (err) {
                ...
                db.query(
                  "INSERT INTO `image_detail`( `prod_id`, `image`, `created_date`, `modify_date`) VALUES  (?,?, NOW(), NOW())",
                  ...
                  (err, result) => {
                    ...
                    if (result) {
                      res.json({
                        status: "1",
                        message: msg_product_image_added,
                      });
                    } else {
                      res.json({ status: "0", message: msg_fail });
                    }
  ...
  app.post("/api/admin/product_image_delete", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["prod_id", "img_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `image_detail` SET `status`= ?, `modify_date` = NOW() WHERE `prod_id`= ? AND `img_id` = ? AND `status` = ? ",
            ...
            (err, result) => {
              ...
              if (result.affectedRows > 0) {
                res.json({
                  status: "1",
                  message: msg_product_image_delete,
                });
              } else {
                res.json({ status: "0", message: msg_fail });
              }
  ...
  app.post("/api/admin/zone_add", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["zone_name"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "SELECT `zone_id`, `name` FROM `zone_detail` WHERE `name`  = ? AND `status` = ?",
            [reqObj.zone_name, "1"],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
              ...
              if (result.length > 0) {
                ...
                db.query(
                  "INSERT INTO `zone_detail`( `name`, `created_date`, `modify_date`) VALUES (?, NOW(), NOW())",
                  [reqObj.zone_name],
                  (err, result) => {
                    if (err) {
                      helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/zone_update", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["zone_id", "zone_name"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `zone_detail` SET `name`= ?, `modify_date` = NOW() WHERE `zone_id`= ? AND `status` = ? ",
  ...
  app.post("/api/admin/zone_delete", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["zone_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `zone_detail` SET `status`= ?, `modify_date` = NOW() WHERE `zone_id`= ? ",
            ["2", reqObj.zone_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/area_add", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["area_name", "zone_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "SELECT `area_id`, `name` FROM `area_detail` WHERE `name`  = ? AND `zone_id` = ? AND `status` = ?",
            [reqObj.area_name, reqObj.zone_id, "1"],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
                return;
              ...
              if (result.length > 0) {
                ...
                db.query(
                  "INSERT INTO `area_detail`( `name`, `zone_id` , `created_date`, `modify_date`) VALUES (?,?, NOW(), NOW())",
  ...
  app.post("/api/admin/area_update", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["area_id", "zone_id", "area_name"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `area_detail` SET `name`= ?, `zone_id` = ? , `modify_date` = NOW() WHERE `area_id`= ? AND `status` = ? ",
  ...
  app.post("/api/admin/area_delete", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["area_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `area_detail` SET `status`= ?, `modify_date` = NOW() WHERE `area_id`= ? ",
            ["2", reqObj.area_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/area_list", (req, res) => {
    ...
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          "SELECT `ad`.`area_id`, `ad`.`zone_id` , `ad`.`name`, `zd`.`name` AS `zone_name`  FROM `area_detail` AS `ad` INNER JOIN `zone_detail` AS `zd` ON `zd`.`zone_id` = `ad`.`zone_id` AND `zd`.`status` = '1' WHERE `ad`.`status`= ? ",
          ["1"],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/offer_add", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["prod_id", "price", "start_date", "end_date"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "INSERT INTO `offer_detail`( `prod_id`, `price`, `start_date`, `end_date`,  `created_date`, `modify_date`) VALUES (?,?,?, ?,NOW(), NOW())",
  ...
  app.post("/api/admin/offer_delete", (req, res) => {
    ...
    helper.CheckParameterValid(res, reqObj, ["offer_id"], () => {
      checkAccessToken(
        ...
        (uObj) => {
          db.query(
            "UPDATE `offer_detail` SET `status`= ?, `modify_date` = NOW() WHERE `offer_id`= ? AND `status` = ? ",
            ["2", reqObj.offer_id, "1"],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/offer_list", (req, res) => {
    ...
    checkAccessToken(
      ...
      (uObj) => {
        db.query(
          "SELECT `offer_id`, `prod_id`, `price`, `start_date`, `end_date`, `status`, `created_date`, `modify_date` FROM `offer_detail` WHERE `status`= ? ",
          ["1"],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
    ...
    checkAccessToken(
      ...
      (userObj) => {
        helper.CheckParameterValid(
          ...
          () => {
            db.query(
              "SELECT `promo_code_id` FROM `promo_code_detail` WHERE `code` = ? AND `status` = 1 ",
              [reqObj.code],
              (err, result) => {
                if (err) {
                  helper.ThrowHtmlError(err, res);
                ...
                if (result.length > 0) {
                  ...
                  db.query(
                    "INSERT INTO `promo_code_detail` ( `code`, `title`, `description`, `type`, `min_order_amount`, `max_discount_amount`, `offer_price`, `start_date`, `end_date`) VALUES (?,?,?, ?,?,?, ?,?,?)",
                    [
                      reqObj.code,
                      reqObj.title,
                      reqObj.description,
                      reqObj.type,
                      reqObj.min_order_amount,
                      reqObj.max_discount_amount,
                      reqObj.offer_price,
                      reqObj.start_date,
                      reqObj.end_date,
                    ],
    ...
    checkAccessToken(
      ...
      (userObj) => {
        helper.CheckParameterValid(
          ...
          () => {
            db.query(
              "UPDATE `promo_code_detail` SET `title`= ? ,`description`= ? ,`type`= ? ,`min_order_amount`= ? ,`max_discount_amount`= ? ,`offer_price`= ? ,`start_date`= ? ,`end_date`= ?, `modify_date` = NOW() WHERE `promo_code_id` = ? AND `start_date` >= NOW() AND `status` = 1 ",
              [
                reqObj.title,
                reqObj.description,
                reqObj.type,
    ...
    checkAccessToken(
      ...
      (userObj) => {
        helper.CheckParameterValid(res, reqObj, ["promo_code_id"], () => {
          db.query(
            "UPDATE `promo_code_detail` SET `status`= 2 , `modify_date` = NOW() WHERE `promo_code_id` = ? AND `status` = 1 ",
            [reqObj.promo_code_id],
            (err, result) => {
              if (err) {
                helper.ThrowHtmlError(err, res);
  ...
  app.post("/api/admin/promo_code_list", (req, res) => {
    ...
    checkAccessToken(
      ...
      (userObj) => {
        db.query(
          "SELECT `promo_code_id`, `code`, `title`, `description`, `type`, `min_order_amount`, `max_discount_amount`, `offer_price`, `start_date`, `end_date`, `created_date`, `modify_date` FROM `promo_code_detail` WHERE `status` = 1 ORDER BY `promo_code_id` DESC ",
          [],
          (err, result) => {
            if (err) {
              helper.ThrowHtmlError(err, res);
        ...
        helper.CheckParameterValid(res, reqObj, ["order_id", "user_id"], () => {
          db.query(
            "SELECT `od`.`order_id`, `od`.`cart_id`, `od`.`total_price`, `od`.`user_pay_price`, `od`.`discount_price`, `od`.`deliver_price`, `od`.`deliver_type`, `od`.`payment_type`, `od`.`payment_status`, `od`.`order_status`, `od`.`status`, `od`.`created_date` FROM `order_detail` AS `od` " +
              ...
              image_base_url +
              "' ,'', `imd`.`image` ) ELSE '' END) AS `image`, (CASE WHEN `od`.`price` IS NULL THEN `pd`.`price` ELSE `od`.`price` END) as `item_price`, ( (CASE WHEN `od`.`price` IS NULL THEN `pd`.`price` ELSE `od`.`price` END) * `ucd`.`qty`)  AS `total_price` FROM `order_detail` AS `uod` " +
    ...
    checkAccessToken(
      ...
      (userObj) => {
        helper.CheckParameterValid(res, reqObj, ["order_id", "user_id", "order_status"], () => {
          db.query(
            "UPDATE `order_detail` SET `order_status`= ?,`modify_date`= NOW() WHERE `order_id` = ? AND `user_id` = ? AND `order_status` < ? ",
            ...
            (err, result) => {
              ...
              if (result.affectedRows > 0) {
                ...
                switch (reqObj.order_status) {
                  case "2":
                    title = "Đã vận chuyển";
                    message = "Đơn hàng " + reqObj.order_id + " của bạn đã được vận chuyển.";
                    break;
                  case "3":
                    title = "Đã giao";
                    message = "Đơn hàng " + reqObj.order_id + " của bạn đã được được giao.";
                    break;
                  case "4":
                    title = "Đã huỷ";
                    message = "Đơn hàng " + reqObj.order_id + " của bạn đã được bị huỷ.";
                    break;
                  case "5":
                    title = "Trả hàng";
                    message = "Đơn hàng " + reqObj.order_id + " của bạn đã được trả lại.";
                    break;
                  default:
                    break;
                }
                ...
                db.query(
                  ...
                  [reqObj.order_id, reqObj.user_id, title, message, notiType],
                  (err, iResult) => {
                    if (err) {
                      helper.ThrowHtmlError(err);
                      return;
